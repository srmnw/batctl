...
static void bisect_v_usage(void)
{
	fprintf(stderr, "Usage: batctl bisect_v [parameters] <file1> <file2> .. <fileN>\n");
...
static int bisect_v_parse_log_file(char *file_path)
{
	FILE *fd;
	char line_buff[MAX_LINE], *start_ptr, *start_ptr_safe, *tok_ptr;
	char *neigh, *iface_addr, *orig, rt_flag;
	int line_count = 0, ttl, i, res, max;
	uint64_t metric, seqno;
	...
	while (fgets(line_buff, sizeof(line_buff), fd) != NULL) {
		...
		if (strstr(start_ptr, "Received OGM2 packet via NB")) {
...
/*			fprintf(stderr, "received packet  (line %i): neigh: '%s', iface_addr: '%s', orig: '%s', seqno: %i, tq: %i, ttl: %i\n", line_count, neigh, iface_addr, orig, seqno, metric, ttl); */
...
/*			printf("route (file: %s, line %i): orig: '%s', neigh: '%s'\n",
			       file_path, line_count, orig, neigh);*/
			...
			res = bisect_routing_table_new(orig, neigh, NULL,
						       rt_flag);
			if (res < 1)
...
int bisect_v(int argc, char **argv)
{
	...
	char orig[NAME_LEN], filter_orig[NAME_LEN], *dash_ptr;
	...
	while ((optchar = getopt(argc, argv, "hl:no:r:s:t:")) != -1) {
		...
		case 'l':
			...
			found_args += ((*(optarg - 1) == optchar ) ? 1 : 2);
			break;
		case 'n':
	...
	if (argc <= found_args + 1) {
		fprintf(stderr,
			"Error - need at least 2 log files to compare\n");
		bisect_v_usage();
		goto err;
	}
	...
	while (argc > found_args) {
		res = bisect_v_parse_log_file(argv[found_args]);
