...
static int netlink_print_meshinfo_json(struct state *state)
{
	int ret;
	struct json_opts json_opts = {
		.is_first = 1,
	...
	ret =  netlink_query_common(state, state->mesh_ifindex,
				    ...
				    meshinfo_callback, 0, &json_opts.query_opts);
	puts("\n");

	return ret;
}

static struct debug_json_data batctl_debug_json_meshinfo = {
	.netlink_fn = netlink_print_meshinfo_json,
};

COMMAND_NAMED(DEBUGJSON, meshinfo_json, "mij", handle_debug_json,
